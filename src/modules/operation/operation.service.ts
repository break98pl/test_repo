...
export class DBOperation {
  ...
  getLoginStaffPermissionConditionService(serviceCode: string) {
    let strQuery = ' ';
    switch (serviceCode) {
      case '5':
        strQuery = `${strQuery}AND (m.権限_高齢者専用住宅版 = '1')`;
        break;
      case '11':
      case 'A2':
      case 'A3':
      case 'A4':
        strQuery = `${strQuery}AND (m.権限_訪問介護 = '1')`;
        break;
      case '12':
      case '62':
        strQuery = `${strQuery}AND (m.権限_訪問入浴 = '1')`;
        break;
      case '13':
      case '63':
        strQuery = `${strQuery}AND (m.権限_訪問看護 = '1')`;
        break;
      case '14':
      case '64':
        strQuery = `${strQuery}AND (m.権限_訪問リハ = '1')`;
        break;
      case '17':
      case '67':
        strQuery = `${strQuery}AND (m.権限_福祉用具 = '1')`;
        break;
      case '31':
      case '34':
        strQuery = `${strQuery}AND (m.権限_療養指導 = '1')`;
        break;
      case '71':
        strQuery = `${strQuery}AND (m.権限_夜間訪問 = '1')`;
        break;
      case '76':
        strQuery = `${strQuery}AND (m.権限_定期巡回随時訪問介護看護 = '1')`;
        break;
      case '15':
      case 'A6':
      case 'A7':
      case 'A8':
        strQuery = `${strQuery}AND (m.権限_通所介護 = '1')`;
        break;
      case '16':
      case '66':
        strQuery = `${strQuery}AND (m.権限_通所リハ = '1')`;
        break;
      case '72':
      case '74':
        strQuery = `${strQuery}AND (m.権限_認知通所 = '1')`;
        break;
      case '78':
        strQuery = `${strQuery}AND (m.権限_地域通所 = '1')`;
        break;
      case '68':
      case '69':
      case '73':
      case '75':
        strQuery = `${strQuery}AND (m.権限_小規模多 = '1')`;
        break;
      case '32':
      case '37':
      case '38':
      case '39':
        strQuery = `${strQuery}AND (m.権限_認知GH = '1' OR m.権限_短期特定 = '1')`;
        break;
      case '33':
      case '27':
      case '35':
        strQuery = `${strQuery}AND (m.権限_特定施設 = '1' OR m.権限_短期認知GH = '1')`;
        break;
      case '21':
      case '51':
      case '24':
        strQuery = `${strQuery}AND (m.権限_短期生活 = '1' OR m.権限_福祉施設 = '1')`;
        break;
      case '22':
      case '52':
      case '25':
        strQuery = `${strQuery}AND (m.権限_短期老健 = '1' OR m.権限_保健施設 = '1')`;
        break;
      case '77':
      case '79':
        strQuery = `${strQuery}AND (m.権限_複合型サービス = '1')`;
        break;
      case '36':
      case '28':
        strQuery = `${strQuery}AND (m.権限_地域特定 = '1')`;
        break;
      case '55':
      case '2A':
      case '2B':
        strQuery = `${strQuery}AND (m.権限_介護医療院 = '1' OR m.権限_短期医療院 = '1')`;
        break;
      case '54':
        strQuery = `${strQuery}AND (m.権限_小規模特養 = '1')`;
        break;
      default:
        strQuery = `${strQuery}AND (m.権限_居宅介護支援版 = '1' OR m.権限_訪問介護 = '1' OR m.権限_訪問入浴 = '1' OR m.権限_訪問看護 = '1' OR m.権限_訪問リハ = '1' OR m.権限_通所介護 = '1' OR m.権限_通所リハ = '1' OR m.権限_福祉用具 = '1' OR m.権限_夜間訪問 = '1' OR m.権限_認知通所 = '1' OR m.権限_小規模多 = '1' OR m.権限_療養指導 = '1' OR m.権限_認知GH = '1' OR m.権限_特定施設 = '1' OR m.権限_短期生活 = '1' OR m.権限_短期老健 = '1' OR m.権限_保健施設 = '1' OR m.権限_福祉施設 ='1')`;
        break;
    }
    return strQuery;
  }
  ...
  async insertTable(tableName: string, items: any[]) {
    ...
    items.forEach(item => {
      ...
      if (tableName === TableName.RoomSchedule) {
      ...
      for (const key of validKeys) {
        if (
          ...
          values.push(`'${sValue.substring(0, 8)}'`);
        } else if (
          tableName === TableName.RoomSchedule &&
          key === 'ベッド番号' &&
          item['FK_部屋コード']
    ...
    if (queries.length > 0) {
      await executeMultiQuery(database, queries, 'insertTable');
    }
  }

  async insertDbWithJsonDict(
    tableName: string,
    items: any[],
    ...
    if (tableName !== TableName.Binary) {
      ...
      if (queries.length > 0) {
        await executeMultiQuery(database, queries, 'insertDbWithJsonDict');
      }
    } else {
  ...
  async getArrayUsersPhotoKey(appType: AppType) {
    const database = await getDBConnection();
    ...
    if (appType === AppType.TSUSHO) {
      strQuery =
        'SELECT 写真データKey FROM M_利用者_個人 LEFT JOIN  T_バイナリ ON T_バイナリ.キー= M_利用者_個人.写真データKey';
    }
    const records = await executeSelectQuery(
      database,
      strQuery,
 ...
 */
  ...
  async getUniqueFkPlanBasesFromReportBase(tenantCodes: string[] = []) {
    return await this.getUniqueFkPlanBasesFromReportBaseWithUserFK(tenantCodes);
  }

  async getUniqueFkPlanBasesFromReportBaseWithUserFK(arrFkKeys: string[]) {
    const database = await getDBConnection();
    let strQuery = `SELECT DISTINCT FK_機能訓練計画書_基本 as fkKey FROM ${TableName.RehaRecord}`;

    if (arrFkKeys && arrFkKeys.length > 0) {
      const strWhere = `WHERE FK_利用者 IN (${arrFkKeys
        .map(e => `'${e}'`)
        .join(',')})`;
  ...
  async getUniqueFkPlanBases(tenantCode = '') {
  ...
  async getUniqueFkPlanBasesWithUserFK(arrFkKeys: string[]) {
    const database = await getDBConnection();
    let strQuery = `SELECT DISTINCT PK_機能訓練計画書_基本 as fkKey FROM ${TableName.RehaSchedule}`;
  ...
  async getUniqueFkReportBases() {
    const strQuery = `SELECT DISTINCT PK_機能訓練記録_基本 as fkKey FROM ${TableName.RehaRecord}`;
    const database = await getDBConnection();
  ...
  async getUniqueFkReportBasesWithFkKey(
    ...
    let strSql = `
      SELECT DISTINCT PK_機能訓練記録_基本 as fkKey 
      FROM ${TableName.RehaRecord} 
      WHERE FK_利用者='${fkKey}'
    `;
    const fromDate = moment(startDate).format(DATE_FORMAT);
    const toDate = moment(endDate).add(1, 'day').format(DATE_FORMAT);

    if (reported) {
      strSql = `${strSql} AND ((対象年月日 < '${toDate}' AND 対象年月日 >= '${fromDate}' ) OR (掲載期限日 >= '${strDateForReport}' AND 期間_選択項目 IS NOT NULL )) `;
    } else {
      strSql = `${strSql} AND (対象年月日 < '${toDate}' AND 対象年月日 >= '${fromDate}') AND (掲載期限日 IS NULL OR 掲載期限日 < '${strDateForReport}') `;
    }
  ...
  /**
   *  Get list attendance by condition string
   *  @param conditionString
   */

  getListAttendanceFromDBWithCondition = async (conditionString: string) => {
    const database = await getDBConnection();
    const strQuery = `
            SELECT 
                T.設定画面ID,
    ...
    const records = await executeSelectQuery(
      database,
      strQuery,
      'getListAttendanceFromDBWithCondition',
    );
    const result: cAttendanceData[] = [];
    if (records && Array.isArray(records) && records.length > 0) {
      await Promise.all(
        records.map(async record => {
          const data = new cAttendanceData(record);
          await data.loadReporterData();
          result.push(data);
        }),
      );
    }
  ...
  /**
   *  Get letter data from DB by todayString and tenantCode
   *  @param conditionString
   */
  async getLetterDataWithTodayString(
    today: string,
    fk_key: string,
  ): Promise<cLetterData[]> {
    const database = await getDBConnection();
    const strQuery = `SELECT T.更新キー, T.設定画面ID, T.サービス種類, T.FK_利用者, T.氏名, T.対象年月日, T.内容, T.職員コード, T.職員名, T.新規フラグ, T.変更フラグ, T.サービス開始日時, T.サービス終了日時  FROM T_日常業務_おたより帳 AS T WHERE T.FK_利用者 = '${fk_key}' AND SubStr(T.対象年月日, 1, 10) = '${moment(
      today,
    ).format('YYYY-MM-DD')}'`;
    ...
    const result: cLetterData[] = [];
    if (records && Array.isArray(records) && records.length > 0) {
      records.forEach(record => {
        const data = new cLetterData(record);
        result.push(data);
      });
    }
    return result;
